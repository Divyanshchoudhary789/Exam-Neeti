...
export function CreateExamModal({
  ...
  // A "public" batch backs a self-serve plan — an exam created against it is
  // effectively "given to that plan / the free tier". Coaching batches are the
  // admin-managed offline cohorts. Show plans first, clearly labelled.
  const PUBLIC_LABEL: Record<string, string> = {
    "public-trial": "Free Trial (free tier)",
    "public-signature-entry": "Signature Entry plan",
    "public-core": "Core plan",
    "public-prime": "Prime plan",
    "public-elite": "Elite plan",
  };
  const publicBatches = batchList.filter((b) => String(b.source) === "public");
  const coachingBatches = batchList.filter((b) => String(b.source) !== "public");
  const batchOptions = [
    ...publicBatches.map((b) => ({
      value: String(b._id || b.id),
      label: `Test-series · ${PUBLIC_LABEL[String(b.slug)] || String(b.name || "Plan")}`,
    })),
    ...coachingBatches.map((b) => ({
      value: String(b._id || b.id),
      label: `Coaching · ${String(b.name || "Batch")}`,
    })),
  ];
