...
export function SprintBuilder({
  ...
  // ── Multi-admin per-subject completion (edit mode) ───────────────────────
  ...
  useEffect(() => {
    ...
    setClassLevel(String(existingSprint.classLevel || ""));
    setSubjectProgress(
      Array.isArray(existingSprint.subjectProgress)
        ? (existingSprint.subjectProgress as SubjectProgress[])
        : []
    );
  ...
  const markSubject = async (subject: string, next: "pending" | "done") => {
    if (!sprintId) return;
    setMarkingSubject(subject);
    try {
      const res = await adminService.markSprintSubject(sprintId, subject, next);
      const sp = res?.data?.sprint?.subjectProgress || res?.sprint?.subjectProgress;
      if (Array.isArray(sp)) setSubjectProgress(sp as SubjectProgress[]);
      showToast(res?.message || `${subject} updated`, "success");
    } catch (err: unknown) {
      showToast((err as { response?: { data?: { message?: string } }; message?: string })?.response?.data?.message
        || (err as { message?: string })?.message || "Failed to update subject", "error");
    } finally {
      setMarkingSubject(null);
    }
  };

  const allSubjectsDone = subjectProgress.length > 0 && subjectProgress.every((s) => s.status === "done");
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <CustomSelect
                    ...
                    buttonClassName="w-full py-3"
                  />
                  <p className="text-[10px] text-slate-400 font-medium mt-1">Used for the class-wise filter on the sprint list.</p>
                </div>
                {isEdit && subjectProgress.length > 0 && (
                  <div className="sm:col-span-2">
                    <Label>Subject Completion</Label>
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {subjectProgress.map((sp) => (
                        <span
                          key={sp.subject}
                          className={classNames(
                            "inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-[11px] font-bold capitalize",
                            sp.status === "done"
                              ? "bg-emerald-50 text-emerald-800 border-emerald-200"
                              : "bg-slate-50 text-slate-600 border-slate-200"
                          )}
                        >
                          {sp.status === "done" ? <IconCheck className="w-3 h-3" /> : <span className="w-1.5 h-1.5 rounded-full bg-amber-400" />}
                          {sp.subject}
                        </span>
                      ))}
                    </div>
                    <p className="text-[10px] text-slate-400 font-medium mt-1">
                      Mark each subject done on the Review step. All subjects must be done before the sprint can go Active.
                    </p>
                  </div>
                )}
              </div>
        ...
        {step === 3 && (
          <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="space-y-3">
              ...
              {isEdit && subjectProgress.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 space-y-2.5">
                  <Label>Subject Completion — multi-admin workflow</Label>
                  <p className="text-[11px] text-slate-500 font-medium leading-relaxed">
                    Each subject admin fills in their slots, then marks their subject done. The sprint can only go Active once all are done.
                  </p>
                  <div className="space-y-1.5">
                    {subjectProgress.map((sp) => {
                      const done = sp.status === "done";
                      return (
                        <div key={sp.subject} className="flex items-center justify-between gap-2 rounded-xl border border-slate-200 px-3 py-2">
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 text-xs font-black text-slate-800 capitalize">
                              <span className={classNames("w-2 h-2 rounded-full shrink-0", done ? "bg-emerald-500" : "bg-amber-400")} />
                              {sp.subject}
                            </div>
                            {done && sp.markedBy?.email && (
                              <div className="text-[10px] text-slate-400 font-semibold truncate">by {sp.markedBy.email}</div>
                            )}
                          </div>
                          <button
                            onClick={() => markSubject(sp.subject, done ? "pending" : "done")}
                            disabled={markingSubject === sp.subject}
                            className={classNames(
                              "shrink-0 px-2.5 py-1.5 rounded-lg text-[11px] font-bold cursor-pointer transition-all disabled:opacity-50",
                              done
                                ? "bg-slate-100 text-slate-600 hover:bg-slate-200"
                                : "bg-emerald-600 text-white hover:bg-emerald-700"
                            )}
                          >
                            {markingSubject === sp.subject ? "…" : done ? "Reopen" : "Mark done"}
                          </button>
                        </div>
                      );
                    })}
                  ...
                  {!allSubjectsDone && (
                    <p className="text-[11px] font-bold text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2.5 py-1.5">
                      Not all subjects are done — saving with status Active will be rejected.
                    </p>
                  )}
                </div>
              )}
