...
export function EditQuestionModal({
  ...
  const [status, setStatus] = useState<"draft" | "active" | "rejected">("active");
  ...
  useEffect(() => {
    if (questionData) {
      ...
      {
        const s = String(questionData.status || "active");
        setStatus(s === "draft" ? "draft" : s === "rejected" ? "rejected" : "active");
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-4 text-slate-800">
            ...
            {activityLog.slice().reverse().map((a, i) => (
              <div key={i} className="flex flex-wrap items-center gap-1.5 text-slate-600">
                <span className="font-bold text-slate-800">
                  {a.action === "reviewed"
                    ? (a.meta?.decision === "reject" ? "Rejected" : "Approved")
                    : (ACTION_LABELS[a.action] || a.action)}
                </span>
                {a.action !== "reviewed" && a.meta?.from && a.meta?.to && (
                  <span className="text-slate-400">({a.meta.from} → {a.meta.to})</span>
                )}
                <span>by</span>
                <span className="font-semibold">{a.byEmail || "unknown"}</span>
                <span className="text-slate-400">· {new Date(a.at).toLocaleString()}</span>
                {a.meta?.note && <span className="w-full text-slate-500 italic pl-1">“{a.meta.note}”</span>}
              </div>
            ))}
          </div>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          ...
          <div>
            ...
            <select
              value={status}
              onChange={(e) => setStatus(e.target.value as "draft" | "active" | "rejected")}
              className={`w-full border text-xs px-3 py-2 rounded-xl focus:outline-none font-bold cursor-pointer ${
                status === "draft" ? "bg-amber-50 border-amber-300 text-amber-800"
                : status === "rejected" ? "bg-red-50 border-red-300 text-red-800"
