...
import {
  IconBook, IconFilter, IconPlus, IconTrash, IconEdit, IconEye, IconSearch, IconCheck, IconKey, IconClock,
...
interface ActivityLogEntry {
  action: "created" | "edited" | "status_changed" | "bulk_uploaded" | "reviewed" | "deactivated" | "reactivated";
  byEmail?: string;
  byRole?: string;
  at: string;
  meta?: { from?: string; to?: string; fileName?: string; decision?: "approve" | "reject"; note?: string };
}
...
const ACTION_LABELS: Record<ActivityLogEntry["action"], string> = {
  ...
  reviewed: "Reviewed", deactivated: "Deactivated", reactivated: "Reactivated",
};

/** Most recent "edited" entry's author, if the question has been edited since creation. */
function getLastEditor(activityLog: unknown): string | null {
...
export function QuestionBankPanel({ showToast }: QuestionBankPanelProps) {
  ...
  const [qStatusFilter, setQStatusFilter] = useState<"active" | "draft" | "rejected" | "">("active");
    ...
    <div className="space-y-5 animate-in fade-in duration-300">
      ...
      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <CustomSelect
            value={qStatusFilter}
            onChange={(val) => { setQStatusFilter(val as "active" | "draft" | "rejected" | ""); setQPage(1); }}
            options={[
            ...
            return (
              <div key={qId} className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/80 hover:border-indigo-200 hover:shadow-md transition-all shadow-sm">
                <div className="flex flex-wrap items-center gap-2 pb-3 border-b border-slate-100">
                  ...
                  {status === "rejected" && (
                    <span className="px-2 py-0.5 rounded-full border text-[10px] font-black uppercase bg-red-50 text-red-700 border-red-200">Rejected</span>
                  )}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mt-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button onClick={() => setHistoryQuestionId(qId)} title="View history" className="p-2 bg-slate-50 hover:bg-slate-200 text-slate-500 border border-slate-200 rounded-xl transition-all cursor-pointer">
                ...
                {(() => {
                  // Shown on every question, not just ones with a recorded creator —
                  // questions inserted by scripts/seedQuestions.js (a direct DB
                  // seed, never through the API) genuinely have no createdBy on
                  // file, so say that plainly instead of just omitting the line.
                  const createdByEmail = (q.createdBy as { email?: string } | undefined)?.email;
                  const lastEditor = getLastEditor(q.activityLog);
                  return (
                    <p className="text-[10px] text-slate-400 font-semibold mt-3 pt-3 border-t border-slate-50">
                      {createdByEmail ? (
                        <>Added by <span className="font-bold text-slate-600">{createdByEmail}</span></>
                      ) : (
                        <span className="italic">Added by — not recorded (seeded before attribution tracking)</span>
                      )}
                      {lastEditor && lastEditor !== createdByEmail && (
                        <> &middot; Last edited by <span className="font-bold text-slate-600">{lastEditor}</span></>
                      )}
                    </p>
                  );
                })()}
        ...
        {fetchingQuestionDetail ? (
          ...
          <div className="space-y-5">
            <div className="flex flex-wrap gap-2 pb-3 border-b border-slate-100">
              ...
              {String(selectedQuestionDetail.status || "active") === "rejected" && (
                <span className="px-2.5 py-1 rounded-full border text-[10px] font-black uppercase bg-red-50 text-red-700 border-red-200">Rejected</span>
              )}
            ...
            {(() => {
              // Always rendered — questions seeded directly via
              // scripts/seedQuestions.js genuinely have no createdBy/activityLog
              // on file, so say so plainly instead of hiding the whole section.
              const createdBy = selectedQuestionDetail.createdBy as { email?: string } | undefined;
              const activityLog = (selectedQuestionDetail.activityLog as ActivityLogEntry[] | undefined) || [];
              return (
                <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                  <span className="text-[10px] font-black uppercase text-slate-500 tracking-wider">Attribution &amp; History</span>
                  <p className="text-xs font-semibold text-slate-700">
                    Added by{" "}
                    {createdBy?.email ? (
                      <span className="font-bold text-slate-900">{createdBy.email}</span>
                    ) : (
                  ...
                  {activityLog.length > 0 ? (
                    <div className="space-y-1.5 pt-2 border-t border-slate-200">
                      {activityLog.slice().reverse().map((a, i) => (
                        <div key={i} className="flex flex-wrap items-center gap-1.5 text-xs text-slate-600">
                          ...
                          <span className="text-slate-400">&middot; {new Date(a.at).toLocaleString()}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-[11px] text-slate-400 italic pt-2 border-t border-slate-200">
                      No edit history recorded for this question.
                    </p>
                  )}
                </div>
              );
